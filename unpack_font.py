...
class Coord:
    ...
    def read(reader):
        ...
        base_x = reader.read_word()  # vertical offset
...
def build_bmp(data, width, height):
    writer = FileWriter()
...
def build_json(ascii_map, coord_arr):
    data = dict()
...
def unpack_font(path):
    ...
    _ = reader.read_int32()  # magic
    ascii_num = reader.read_int32()
    glyph_num = reader.read_int32()

    ascii_map = {i: reader.read_byte() for i in range(ascii_num)}
    coord_arr = {i: Coord.read(reader) for i in range(glyph_num)}

    _ = reader.read_int32()  # unk
    img_width = reader.read_int32()
    img_height = reader.read_int32()
    image = reader.read(img_width * img_height)

    bmp = build_bmp(image, img_width, img_height)
    save_binary(path + ".bmp", bmp)

    json_list = build_json(ascii_map, coord_arr)
